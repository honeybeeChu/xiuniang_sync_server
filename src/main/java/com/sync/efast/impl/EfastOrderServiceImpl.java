...
public class EfastOrderServiceImpl implements EfastOrderService {
	...
	// getEfastOrdersFromLastTimeByPage(0,new Date(),efastOrderList);
	// System.out.println(efastOrderList.get(0).getBuyer_name());
	// } catch (Exception e) {
	 ...
	 */
	@Override
	public void getEfastOrdersFromLastTimeByPage(int pageIndex, Date currentDate, List<Efast_order> efastOrderList) {
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if ("1".equals(jsonobj.get("status").toString())) {
					...
					for (int i = 0; i < orderArray.size(); i++) {
						...
						efastOrderList.add(new Efast_order(jsonObject).createEfastOrder());
					}
					// 是最后一页了
	...
	@Override
	public void getEfastOrdersFromTimeToTime(int pageIndex, String startTime, String endTime,
			List<Efast_order> efastOrderList) {
		try {
			String efastHttpUrl = getConditionParamsStr(pageIndex, startTime,endTime);
			CloseableHttpClient httpclient = HttpClients.createDefault();
			HttpGet httpGet = new HttpGet(efastHttpUrl);
			CloseableHttpResponse response1 = httpclient.execute(httpGet);
			HttpEntity entity1 = response1.getEntity();
			...
			while ((line = br.readLine()) != null) {
				...
				if ("1".equals(jsonobj.get("status").toString())) {
					...
					// 是最后一页了
					if (pageIndex != Integer.parseInt(filterObj.get("page_count").toString())) {
						getEfastOrdersFromTimeToTime(++pageIndex, startTime,endTime, efastOrderList);
					}
				}
			}
			EntityUtils.consume(entity1);
	...
	private static String getConditionParamsStr(int pageIndex, String timestamp) throws Exception {
		...
		// String efast_url =
		// PropertyPlaceholder.getProperty("efast_url").toString();
		paramBuf.append(efast_url).append("&format=json&key=").append("f2889c8118cada8a52965d087da5d088")
				...
				// 最后更新时间社则为现在
				// .append("&end_lastchanged=").append(URLEncoder.encode(timestamp,
				// "UTF-8"))
				// 已发货
	...
	//manual controller used
	private static String getConditionParamsStr(int pageIndex, String startTime,String endTime) throws Exception {
		StringBuffer paramBuf = new StringBuffer();
		String efast_url = "http://openapi.baotayun.com/openapi/webefast/web/?app_act=openapi/router";
		...
		paramBuf.append(efast_url).append("&format=json&key=").append("f2889c8118cada8a52965d087da5d088")
				...
				// 最后更新时间的开始范围是：Constant.lastSyncTime
				.append("&start_lastchanged=").append(URLEncoder.encode(startTime, "UTF-8"))
				// 最后更新时间社则为现在
				 .append("&end_lastchanged=").append(URLEncoder.encode(endTime,
				 "UTF-8"))
				// 已发货
				.append("&shipping_status=4")
				// sign参数
				.append("&sign=").append(getSign(pageIndex, startTime,endTime,timestamp))
				// sign_method加密方式
				.append("&sign_method=").append("md5")
				// order_status参数，订单状态为已确认
				.append("&order_status=").append("1")
	...
	//manual controller used
	private static String getSign(int pageIndex, String startTime,String endTime,String timestamp) throws Exception {
		
		StringBuffer signBuf = new StringBuffer();
		// signBuf.append(PropertyPlaceholder.getProperty("efast_secret").toString())
		signBuf.append("1feb581b4302688911584998fd8e66d9")
				.append("end_lastchanged").append(endTime)
				.append("formatjsonkeyf2889c8118cada8a52965d087da5d088methodoms.order.search.get")
				.append("order_status1").append("page").append(pageIndex)
				.append("shipping_status4sign_methodmd5start_lastchanged").append(Constant.getlastSyncTimeAsStr())
				.append("timestamp").append(timestamp).append("v2.0").append("1feb581b4302688911584998fd8e66d9");
